...
function checkEmailValidate(email) {
    let emailPattern = /\d{5}@\w{3}admin$/;
    ...
    if (emailPattern.test(email)) {
        document.getElementById("emailHelp1").textContent = 'Mã đúng định dạng';
        document.getElementById("emailHelp1").style.color = 'grey';
        check1 = true;
    } else {
        document.getElementById("emailHelp1").textContent = 'Mã admin bắt đầu bằng 5 số trước @, 3 kí tự chữ sau @, và kết thúc bằng "admin"';
...
function Func1() {
    ...
    if (check1 == true & check2 == true)
        homeBTN.href = "../admin-product-list/admin-product-list.html";
}
